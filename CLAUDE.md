...
- **B9 content-management layer (CMS) in progress.** Mobile responsive nav (slide-out drawers, customer + admin) shipped. **Content cache (PART 0):** owner-managed content + settings are read cache-first via `src/lib/content/` — `cache.ts` defines the tags + `cachedContent`/`revalidateContent`, `reads.ts` wraps the DAL. Customer pages (home, services, prices, booking flow) read through the cache, so a page load does **not** hit the DB per request. **Cache tags** (a read's tag == the tag its admin editor revalidates): `catalog` (services, tiers, price matrix, add-ons, deposit), `service-area` (postcode areas + travel fees), `homepage` (hero copy + before/after media), `reviews` (testimonials), `legal` (privacy/terms/cookie text), `business` (legal name, address, VAT, contact). Every admin mutation calls `revalidateContent(<tag>)` (+ `revalidatePath` where a route must repaint). Coming next: admin CRUD for catalog+deposit (A), service area (B), homepage+media (C), reviews (D), legal+business (E).
